...
void LCD_SendCommand(u8 cmd) {
	if (mode == 8) {
		...
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN4, GET_BIT(cmd, 4));
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN5, GET_BIT(cmd, 5));
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN6, GET_BIT(cmd, 6));
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN7, GET_BIT(cmd, 7));
		...
		if (modefinish) {
			DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN4, GET_BIT(cmd, 0));
			DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN5, GET_BIT(cmd, 1));
			DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN6, GET_BIT(cmd, 2));
			DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN7, GET_BIT(cmd, 3));
...
void LCD_SendChar(u8 Char) {
	...
	if (mode == 8) {
		...
		DIO_SetPinValue(LCD_CONT_PORT, LCD_RW_PIN, DIO_LOW); //RW = 0
		DIO_SetPinValue(LCD_CONT_PORT, LCD_RS_PIN, DIO_HIGH); //RS = 1

		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN4, GET_BIT(Char, 4)); //PIN4
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN5, GET_BIT(Char, 5)); //PIN5
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN6, GET_BIT(Char, 6)); //PIN6
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN7, GET_BIT(Char, 7)); //PIN7
		//first 4 bits then enable to make him understand that
		//there are data being processed

		DIO_SetPinValue(LCD_CONT_PORT, LCD_E_PIN, DIO_HIGH);
		_delay_ms(1);
		DIO_SetPinValue(LCD_CONT_PORT, LCD_E_PIN, DIO_LOW);

		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN4, GET_BIT(Char, 0));
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN5, GET_BIT(Char, 1));
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN6, GET_BIT(Char, 2));
		DIO_SetPinValue(LCD_DATA_PORT, DIO_PIN7, GET_BIT(Char, 3));
